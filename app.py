...
def date_range_selector(data_dict, user_id):
    ...
    # Handle potential mixed types in user_dates
    if user_dates:
        try:
            min_date = min(d for d in user_dates if not pd.isna(d))
            max_date = max(d for d in user_dates if not pd.isna(d))
            ...
            # If there's a type error, convert all to the same type
            clean_dates = []
            for d in user_dates:
                if pd.isna(d):
                    continue
                if isinstance(d, pd.Timestamp):
                    ...
                    try:
                        ...
                        clean_dates.append(pd.to_datetime(d).date())
                    except:
                        pass
            if clean_dates:
...
def activity_analysis_page(data_dict, user_id, date_range):
    ...
    filtered_daily = daily_df[(daily_df['Id'] == user_id) & 
        (pd.to_datetime(daily_df['Date']) >= start_date_ts) & 
        (pd.to_datetime(daily_df['Date']) <= end_date_ts)]
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            if 'TotalSteps' in filtered_daily.columns:
                ...
                steps_col = [col for col in filtered_daily.columns if col.lower() == 'steps' or col.lower() == 'totalsteps']
                if steps_col:
    ...
    with tabs[1]:
        ...
        # Create a fresh copy of the data for weekly analysis
        weekly_data = filtered_daily.copy()
        
        # Define day order FIRST to avoid the UnboundLocalError
        day_order = {
            'Monday': 0, 
            'Tuesday': 1, 
            'Wednesday': 2, 
        ...
        # Ensure we have the Day column set up correctly
        if 'Day' not in weekly_data.columns or weekly_data['Day'].isna().any():
            weekly_data['Date'] = pd.to_datetime(weekly_data['Date'])
            weekly_data['Day'] = weekly_data['Date'].dt.day_name()
        
        if 'DayOfWeek' not in weekly_data.columns:
            weekly_data['DayOfWeek'] = weekly_data['Day'].map(day_order)
        
        # Calculate average steps by day
        if 'TotalSteps' in weekly_data.columns:
            steps_by_day = weekly_data.groupby('Day')['TotalSteps'].mean().reset_index()
            
            # Ensure all days are present
            existing_days = steps_by_day['Day'].tolist()
            for day in day_names:
                if day not in existing_days:
                    # Add missing day with zero values
                    steps_by_day = steps_by_day.append({'Day': day, 'TotalSteps': 0}, ignore_index=True)
            
            # Now add the DayOfWeek column for sorting
            steps_by_day['DayOfWeek'] = steps_by_day['Day'].map(day_order)
            steps_by_day = steps_by_day.sort_values('DayOfWeek')
            ...
            fig = px.bar(
                ...
                labels={'TotalSteps': 'Average Steps', 'Day': 'Day of Week'},
            ...
            # Calculate detailed statistics by day
            weekly_stats = {}
            for day in day_names:
                day_data = weekly_data[weekly_data['Day'] == day]
                ...
                # Calculate statistics if we have data for this day
                if not day_data.empty:
                    if 'TotalSteps' in day_data.columns:
                        day_stats['Avg Steps'] = day_data['TotalSteps'].mean()
                        day_stats['Median Steps'] = day_data['TotalSteps'].median()
                        day_stats['Max Steps'] = day_data['TotalSteps'].max()
                    
                    if 'VeryActiveMinutes' in day_data.columns:
                        day_stats['Avg Very Active'] = day_data['VeryActiveMinutes'].mean()
                    
                    if 'FairlyActiveMinutes' in day_data.columns:
                        day_stats['Avg Fairly Active'] = day_data['FairlyActiveMinutes'].mean()
                    
                    if 'Calories' in day_data.columns:
                        day_stats['Avg Calories'] = day_data['Calories'].mean()
                
                weekly_stats[day] = day_stats
            
            # Create a DataFrame from weekly stats
            stats_df = pd.DataFrame(weekly_stats.values())
            
            # Add day order for sorting
            stats_df['day_sort'] = stats_df['Day'].map(day_order)
            stats_df = stats_df.sort_values('day_sort').drop(columns=['day_sort'])
            
            # Round numeric columns
            for col in stats_df.columns:
                if col != 'Day' and stats_df[col].dtype.kind in 'if':
            ...
            if len(steps_by_day) > 0:
                try:
                    max_row = steps_by_day.loc[steps_by_day['TotalSteps'].idxmax()]
                    min_row = steps_by_day.loc[steps_by_day['TotalSteps'].idxmin()]
                    
                    most_active_day = max_row['Day']
                    least_active_day = min_row['Day']
                    ...
                    st.warning(f"Could not determine most/least active days: {str(e)}")
        else:
            st.warning("Steps data not available for weekly pattern analysis.")
    ...
    with tabs[2]:
        ...
        # Determine which steps column to use
        steps_col = None
        if 'TotalSteps' in filtered_daily.columns:
            steps_col = 'TotalSteps'
        elif 'Steps' in filtered_daily.columns:
            steps_col = 'Steps'
        
        if steps_col:
            # Make a clean copy of the data
            step_data = filtered_daily.copy()
            
            # Calculate achievement stats
            steps_goal = 10000  # Default goal
            days_over_goal = (step_data[steps_col] >= steps_goal).sum()
            ...
            with col1:
                ...
                streak_data = step_data.sort_values('Date')
                streak_data['goal_achieved'] = streak_data[steps_col] >= steps_goal
                ...
                if not streaks.empty:
                    goal_achieved_streaks = streaks[streaks['goal_achieved']]
                    if not goal_achieved_streaks.empty:
                        current_streak = goal_achieved_streaks.iloc[-1]['streak_length']
                        max_streak = goal_achieved_streaks['streak_length'].max()
            ...
            # New section: Step Progress Analysis
            st.markdown('<div class="sub-header">Step Progress Analysis</div>', unsafe_allow_html=True)
            
            # Calculate progress metrics
            if len(step_data) > 1:
                # Calculate moving averages
                step_data['7_Day_Avg'] = step_data[steps_col].rolling(window=7, min_periods=1).mean()
                
                # Calculate weekly improvement
                step_data['Weekly_Change'] = step_data['7_Day_Avg'].diff(7)
                
                # Create line chart showing progress over time with 7-day average
                fig = px.line(
                    step_data.sort_values('Date'),
                    x='Date',
                    y=[steps_col, '7_Day_Avg'],
                    title='Step Count Progress Over Time',
                    labels={
                        steps_col: 'Daily Steps',
                        '7_Day_Avg': '7-Day Average',
                        'Date': 'Date'
                    },
                    color_discrete_map={
                        steps_col: 'rgba(66, 133, 244, 0.7)',
                        '7_Day_Avg': 'rgba(219, 68, 55, 0.9)'
                    }
                )
                ...
                fig.add_shape(
                    type='line',
                    x0=step_data['Date'].min(),
                    y0=steps_goal,
                    x1=step_data['Date'].max(),
                    y1=steps_goal,
                    line=dict(color='green', width=2, dash='dash'),
                )
                
                fig.add_annotation(
                    x=step_data['Date'].max(),
                    y=steps_goal,
                    text='Goal',
                    showarrow=False,
                ...
                with col1:
                    latest_avg = step_data['7_Day_Avg'].dropna().iloc[-1] if not step_data['7_Day_Avg'].dropna().empty else 0
                    first_avg = step_data['7_Day_Avg'].dropna().iloc[0] if not step_data['7_Day_Avg'].dropna().empty else 0
                    progress = latest_avg - first_avg
                    progress_pct = (progress / first_avg * 100) if first_avg > 0 else 0
                    
                    st.metric(
                        label="Overall Progress", 
                        value=f"{latest_avg:.0f} steps",
                        delta=f"{progress_pct:.1f}%" if first_avg > 0 else None
                    )
                
                with col2:
                    # Calculate consistency - standard deviation relative to mean
                    cv = step_data[steps_col].std() / step_data[steps_col].mean() * 100 if step_data[steps_col].mean() > 0 else 0
                    consistency_score = max(0, 100 - cv)
                    
                    st.metric(
                        label="Consistency Score",
                        value=f"{consistency_score:.1f}%",
                        help="Higher score means more consistent daily steps"
                    )
                
                with col3:
                    # Calculate goal achievement rate trend
                    if len(step_data) >= 14:
                        recent_week = step_data.sort_values('Date').tail(7)
                        previous_week = step_data.sort_values('Date').tail(14).head(7)
                        
                        recent_achievement = (recent_week[steps_col] >= steps_goal).mean() * 100
                        previous_achievement = (previous_week[steps_col] >= steps_goal).mean() * 100
                        
                        achievement_change = recent_achievement - previous_achievement
                        
                        st.metric(
                            label="Weekly Goal Rate",
                            value=f"{recent_achievement:.1f}%",
                            delta=f"{achievement_change:.1f}%"
                        )
                    else:
                        overall_rate = (step_data[steps_col] >= steps_goal).mean() * 100
                        st.metric(
                ...
                st.markdown('<div class="sub-header">Step Count Distribution Analysis</div>', unsafe_allow_html=True)
                
                col1, col2 = st.columns(2)
                
                with col1:
                    # Create step range categories
                    step_ranges = [
                        (0, 2500, 'Sedentary'),
                        (2500, 5000, 'Low Active'),
                        (5000, 7500, 'Somewhat Active'),
                        (7500, 10000, 'Active'),
                        (10000, float('inf'), 'Very Active')
                    ]
                    
                    # Categorize each day
                    step_data['ActivityLevel'] = 'Unknown'
                    for low, high, label in step_ranges:
                        mask = (step_data[steps_col] >= low) & (step_data[steps_col] < high)
                        step_data.loc[mask, 'ActivityLevel'] = label
                    
                    # Calculate category counts
                    category_counts = step_data['ActivityLevel'].value_counts().reset_index()
                    category_counts.columns = ['Category', 'Count']
                    
                    # Order categories from sedentary to very active
                    category_order = {
                        'Sedentary': 0,
                        'Low Active': 1,
                        'Somewhat Active': 2,
                    ...
                    category_counts['Sort'] = category_counts['Category'].map(category_order)
                    category_counts = category_counts.sort_values('Sort')
                    ...
                    fig = px.pie(
                        ...
                        values='Count',
                        names='Category',
                        title='Step Activity Level Distribution',
                        color='Category',
                        color_discrete_map={
                            'Sedentary': '#E57373',
                            'Low Active': '#FFB74D',
                            'Somewhat Active': '#FFF176',
                            'Active': '#81C784',
                            'Very Active': '#4CAF50'
                        }
                    )
                ...
                with col2:
                    ...
                    percentile_values = np.percentile(step_data[steps_col], percentiles)
                    
                    # Create a table of percentiles
                    percentile_df = pd.DataFrame({
                        'Percentile': [f"{p}%" for p in percentiles],
                        'Steps': percentile_values.round(0)
                    })
                    
                    st.table(percentile_df)
                    
                    # Determine normal step range (25th to 75th percentile)
                    normal_range = f"{percentile_values[1]:.0f} - {percentile_values[3]:.0f}"
                    st.info(f"Your normal step range (25th-75th percentile): **{normal_range}**")
                    
                    # Show exceptional days
                    top_day = step_data.loc[step_data[steps_col].idxmax()]
                    st.success(f"Your most active day was **{top_day['Date'].strftime('%Y-%m-%d')}** with **{top_day[steps_col]:.0f}** steps")
            else:
                st.warning("Not enough data for progress analysis.")
        else:
            st.warning("Step data not available for the selected user.")
    ...
    with tabs[4]:
        ...
        if merged_daily_df is not None:
            ...
            if not filtered_merged.empty:
                ...
                # For the Sleep and Activity Correlation section
                if 'TotalMinutesAsleep' in filtered_merged.columns:
                    # Only show the section if we have sleep data
                    if not filtered_merged['TotalMinutesAsleep'].isna().all() and len(filtered_merged) >= 5:
                        ...
                        if sleep_corr and 'same_day_correlations' in sleep_corr and sleep_corr['same_day_correlations']:
                            col1, col2 = st.columns(2)
                            ...
                            with col2:
                                ...
                                prev_day_data = []
                                if 'previous_day_correlations' in sleep_corr:
                                ...
                                if prev_day_data:
                                    ...
                                    st.info("No previous day correlation data available")
                            ...
                            st.info("Not enough data to calculate sleep-activity correlations")
                    else:
                        st.info("Not enough sleep data available for correlation analysis")

                # For the Calories Prediction Analysis section
                if 'Calories' in filtered_merged.columns:
                    # Only show this section if we have enough data points
                    if len(filtered_merged) >= 10 and not filtered_merged['Calories'].isna().all():
                        ...
                        if model and importance and metrics:
                            ...
                            st.info("Could not build a prediction model with the available data")
...
def sleep_insights_page(data_dict, user_id, date_range):
    ...
    with tabs[0]:
        ...
        if isinstance(avg_sleep_min, pd.Timestamp) or pd.isna(avg_sleep_min):
            avg_sleep_min = 0
        avg_sleep_hours = float(avg_sleep_min) / 60
        
        avg_time_in_bed_min = filtered_sleep['TotalTimeInBed'].mean()
        if isinstance(avg_time_in_bed_min, pd.Timestamp) or pd.isna(avg_time_in_bed_min):
    ...
    with tabs[1]:
        ...
        if 'SleepDay' in filtered_sleep.columns:
            # FIX: Ensure TotalTimeInBed and TotalMinutesAsleep are numeric
            filtered_sleep = filtered_sleep.copy()
            filtered_sleep['TotalTimeInBed'] = pd.to_numeric(filtered_sleep['TotalTimeInBed'], errors='coerce')
            filtered_sleep['TotalMinutesAsleep'] = pd.to_numeric(filtered_sleep['TotalMinutesAsleep'], errors='coerce')
        ...
        if 'Day' in filtered_sleep.columns:
            ...
            if not sleep_by_day.empty:
                try:
                    most_sleep_idx = sleep_by_day['TotalMinutesAsleep'].idxmax()
                    least_sleep_idx = sleep_by_day['TotalMinutesAsleep'].idxmin()

                    most_sleep_day = sleep_by_day.loc[most_sleep_idx, 'Day']
                    least_sleep_day = sleep_by_day.loc[least_sleep_idx, 'Day']
                    ...
                    st.warning(f"Could not identify most/least sleep days: {str(e)}")
            else:
                st.info("Not enough data to identify sleep patterns by day")
    ...
    with tabs[1]:
        ...
        if not selected_hr.empty:
            ...
            if 'Time' in selected_hr.columns:
                ...
                # Only process rows where Time is not NaT
                valid_time_mask = ~selected_hr['Time'].isna()
                if valid_time_mask.any():
                    selected_hr.loc[valid_time_mask, 'Hour'] = selected_hr.loc[valid_time_mask, 'Time'].dt.hour
                ...
                st.warning(f"Time data not available for {selected_date}.")
        else:
    ...
    with tabs[3]:
        ...
        if anomalies is not None and not anomalies.empty:
            ...
            if anomaly_dates:
                ...
                # Check if 'Time' column exists before sorting
                if 'Time' in date_anomalies.columns:
                    date_anomalies = date_anomalies.sort_values('Time')
                else:
                    # Use index or another appropriate column if 'Time' doesn't exist
                    date_anomalies = date_anomalies.sort_index()
                
                # Create a plot showing heart rate with anomalies highlighted
                fig = go.Figure()
                
                # Add all heart rate points
                x_values = date_anomalies['Time'] if 'Time' in date_anomalies.columns else date_anomalies.index
                ...
                if not anomaly_points.empty:
                    x_anomaly = anomaly_points['Time'] if 'Time' in anomaly_points.columns else anomaly_points.index
                ...
                fig.update_layout(
                    ...
                    xaxis_title='Time' if 'Time' in date_anomalies.columns else 'Index',
                    yaxis_title='Heart Rate (bpm)',
                    legend=dict(
                        orientation='h',
                        yanchor='bottom',
...
def weight_management_page(data_dict, user_id, date_range):
    ...
    with tabs[2]:
        ...
        if daily_activity_df is not None and 'Calories' in daily_activity_df.columns:
            ...
            available_cols = ['Date']
            col_mapping = {
                'TotalSteps': ['TotalSteps', 'TotalSteps', 'totalsteps', 'steps'],
                'Calories': ['Calories', 'calories'],
                'VeryActiveMinutes': ['VeryActiveMinutes', 'veryactiveminutes'],
                'FairlyActiveMinutes': ['FairlyActiveMinutes', 'fairlyactiveminutes'],
            }
