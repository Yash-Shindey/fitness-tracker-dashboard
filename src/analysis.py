...
def analyze_weekly_patterns(daily_activity_df):
    ...
    # Check if Day and DayOfWeek columns exist, if not, create them
    if 'DayOfWeek' not in daily_activity_df.columns:
        if 'Day' in daily_activity_df.columns:
            # Map day names to day numbers
            day_map = {
                'Monday': 0, 'Tuesday': 1, 'Wednesday': 2, 'Thursday': 3,
                'Friday': 4, 'Saturday': 5, 'Sunday': 6
            }
            daily_activity_df = daily_activity_df.copy()
            daily_activity_df['DayOfWeek'] = daily_activity_df['Day'].map(day_map)
        elif 'Date' in daily_activity_df.columns:
            # Create DayOfWeek from Date
            daily_activity_df = daily_activity_df.copy()
            daily_activity_df['Date'] = pd.to_datetime(daily_activity_df['Date'])
            daily_activity_df['DayOfWeek'] = daily_activity_df['Date'].dt.dayofweek
            daily_activity_df['Day'] = daily_activity_df['Date'].dt.day_name()
        else:
            # If we can't create DayOfWeek, return None
            return None
    elif 'Day' not in daily_activity_df.columns:
        # If DayOfWeek exists but Day doesn't, create Day
        day_names = {
            0: 'Monday',
            1: 'Tuesday',
            2: 'Wednesday',
        ...
        daily_activity_df = daily_activity_df.copy()
        daily_activity_df['Day'] = daily_activity_df['DayOfWeek'].map(day_names)
    ...
    # Don't attempt to reorder MultiIndex columns
    # Just return the patterns as is
