...
def create_activity_summary_cards(daily_activity_df, user_id=None, date_range=None):
    ...
    if 'Steps' in filtered_df.columns:
        summary['avg_steps'] = int(filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'].mean())
        summary['max_steps'] = int(filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'].max())
        summary['total_steps'] = int(filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'].sum())
        
        # Goal achievement (assuming 10,000 steps goal)
        goal_achievement = (filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'] >= 10000).mean() * 100
...
def plot_daily_steps_trend(daily_activity_df, user_id=None, date_range=None, rolling_window=7):
    ...
    filtered_df['RollingSteps'] = filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'].rolling(window=rolling_window, min_periods=1).mean()
    ...
    fig.add_trace(go.Scatter(
        x=filtered_df['Date'],
        y=filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'],
        mode='markers',
        name='Daily Steps',
        marker=dict(
            size=8,
...
def plot_activity_heatmap(hourly_steps_df, user_id=None, date_range=None):
    ...
    if date_range:
        ...
        # Filter between the timestamps
        filtered_df = filtered_df[filtered_df['ActivityHour'].notna()]
        filtered_df = filtered_df[(filtered_df['ActivityHour'] >= start_date_ts) & 
                                  (filtered_df['ActivityHour'] <= end_date_ts)]
...
def plot_sleep_patterns(sleep_df, user_id=None, date_range=None, rolling_window=7):
    ...
    # Handle potential datetime objects and convert minutes to hours for better readability
    if 'TotalMinutesAsleep' in filtered_df.columns:
        # Check if it's a datetime type
        if pd.api.types.is_datetime64_dtype(filtered_df['TotalMinutesAsleep']):
            # If it's a datetime, convert it to numeric
            filtered_df['SleepHours'] = pd.to_numeric(filtered_df['TotalMinutesAsleep'].astype(str).str.extract('(\d+)')[0], errors='coerce') / 60
        else:
            # If it's already numeric
            filtered_df['SleepHours'] = filtered_df['TotalMinutesAsleep'] / 60
    
    if 'TotalTimeInBed' in filtered_df.columns:
        # Check if it's a datetime type
        if pd.api.types.is_datetime64_dtype(filtered_df['TotalTimeInBed']):
            # If it's a datetime, convert it to numeric
            filtered_df['TimeInBedHours'] = pd.to_numeric(filtered_df['TotalTimeInBed'].astype(str).str.extract('(\d+)')[0], errors='coerce') / 60
        else:
            # If it's already numeric
...
def create_achievement_badges(daily_activity_df, user_id=None):
    ...
    # Steps badges
    if 'Steps' in filtered_df.columns:
        max_steps = filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'].max()
        if max_steps >= 20000:
        ...
        # Consistency badge
        steps_consistency = (filtered_df['TotalSteps'] if 'TotalSteps' in df.columns else df['TotalSteps'] if 'TotalSteps' in df.columns else df['Steps'] >= 10000).mean() * 100
